...
**Feature Branch**: `001-festival-timeline-app`
**Created**: 2025-10-18
**Status**: Draft
...
- Q: How should users authenticate to access the festival timeline app? → A: Email/password only (internal use)
- Q: Who can create and modify event categories in the system? → A: Anyone can create categories
- Q: What functionality should be available when users lose internet connectivity? → A: Require constant connection
- Q: How do new users join and get access to the festival timeline? → A: Open self-registration
- Q: Can users create multiple festival timelines, or is there a single shared timeline? → A: Single shared timeline - one source of truth
...
- What happens when two users edit the same event simultaneously?
  - System uses last-write-wins strategy with optimistic locking via updatedAt timestamps
  - User must refresh page to see latest changes
...
- What happens when a user loses internet connection?
  - App displays a clear error message and blocks all functionality until connection is restored
...
- **FR-010**: System MUST store all event data in the database for team collaboration
- **FR-011**: System MUST support user authentication via email/password
...
- **FR-015**: System MUST handle concurrent edits gracefully with last-write-wins strategy
- **FR-016**: System MUST be accessible via web browsers (Chrome, Firefox, Safari, Edge)
- **FR-017**: System MUST be responsive and work on desktop, tablet, and mobile devices
- **FR-018**: System MUST allow any authenticated user to create, edit, and delete event categories
- **FR-019**: System MUST allow open self-registration where any user can sign up and immediately gain access
- **FR-020**: System MUST provide a single shared timeline where all users collaborate on the same set of events (one source of truth)

### Key Entities

**Note**: The system maintains a single shared timeline - all users collaborate on the same set of events, categories, and data. There is no concept of separate timelines or workspaces.
...
- **Event**: Represents a task or milestone in the festival planning process
  ...
  - Scope: Global - all events are visible to all users

- **Category**: Represents a type or area of festival activity
  - Attributes: id, name, color (for visual distinction on timeline), createdBy, createdAt
  - Examples: Venue Setup, Marketing, Logistics, Entertainment, Food & Beverage, Security, Registration, Cleanup
  - Note: Any user can create, edit, and delete categories
  - Scope: Global - all categories are visible to all users

- **User**: Represents a team member who can create, edit, and view events
  - Attributes: id, name, email
  - Access: All users have full access to the single timeline
...
- **SC-008**: Reduce festival planning meeting time by 25% through improved visibility and organization
- **SC-009**: Timeline zoom supports 3 view levels (day, week, month) with smooth transitions
- **SC-010**: Timeline horizontal scrolling maintains smooth performance with pan gesture support on touch devices
